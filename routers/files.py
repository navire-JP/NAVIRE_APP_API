...
# =========================
# Structure: file_id -> {"file_id": str, "filename": str, "page_count": Optional[int], "path": str|None}
FILES_DB: Dict[str, Dict[str, Any]] = {}

def _scan_storage_once() -> None:
    """
    Recharge un index minimal depuis le disque (ignoré en mode mock).
    Nom sur disque: "<uuid>_<filename>" pour conserver le nom d'origine.
    """
    if USE_FAKE:
        return
    ...
    for p in STORAGE_DIR.glob("*"):
        ...
        # Essayer d'extraire l'UUID en préfixe si présent
        ...
        try:
            ...
            # pas au format <uuid>_<filename>, on garde tel quel
            pass

        page_count: Optional[int] = None
        if filename.lower().endswith(".pdf"):
            page_count = _safe_count_pdf_pages(p)
...
def _current_store() -> Dict[str, Dict[str, Any]]:
    if USE_FAKE:
        # convertir la liste en dict file_id -> item
...
async def list_files() -> List[FileListItem]:
    """
    Retourne une liste de fichiers au format:
    [
      { "file_id": "...", "filename": "...", "page_count": 12|None },
      ...
    ]
    """
    store = _current_store()
    res: List[FileListItem] = []
    for item in store.values():
        res.append(FileListItem(
...
@router.post(
    "/upload",
    summary="Uploader un fichier (multipart)",
...
async def upload_file(file: UploadFile = File(...)) -> FileListItem:
    ...
    if USE_FAKE:
        ...
        return FileListItem(file_id=file_id, filename=original_name, page_count=None)

    target = STORAGE_DIR / f"{file_id}_{original_name}"
    try:
        with open(target, "wb") as out:
            chunk = await file.read()  # suffisant pour des fichiers moyens
            out.write(chunk)
    ...
    return FileListItem(file_id=file_id, filename=original_name, page_count=page_count)
...
async def get_file_meta(file_id: str) -> FileListItem:
    store = _current_store()
